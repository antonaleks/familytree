...
test('buildEdges: ребёнок от союза-сердечка, супруги без стрелки', () => {
  const g = sample();
  const edges = buildEdges(g);
  // оба родителя в браке → стрелка идёт от союза u-dad|mom, не от каждого
  const uid = 'u-' + ['dad', 'mom'].sort().join('|');
  const pc = edges.find(e => e.source === uid && e.target === 'kid');
  assert.ok(pc, 'есть связь союз→ребёнок');
  assert.ok(pc.markerEnd, 'у связи союз→ребёнок есть markerEnd');
  assert.equal(edges.find(e => e.source === 'dad' && e.target === 'kid'), undefined,
    'прямой связи родитель→ребёнок нет (идёт через союз)');
  const sp = edges.find(e => e.id.startsWith('s-'));
  assert.equal(sp.markerEnd, undefined, 'у супружеской связи нет стрелки');
  // 2 пары × 2 половины (супруг→союз→супруг) = 4 ребра брака
  const spCount = edges.filter(e => e.id.startsWith('s-')).length;
  assert.equal(spCount, 4); // gf-gm и dad-mom, по 2 половины
});
...
test('buildLayout: карточки не налезают (уникальные позиции) и пары рядом', () => {
  ...
  // союзы-сердечки добавлены: 2 пары (gf-gm, dad-mom)
  assert.equal(nodes.filter(n => n.type === 'union').length, 2);
  // нет двух карточек в одной точке
  const keys = persons.map(n => `${n.position.x},${n.position.y}`);
  assert.equal(new Set(keys).size, keys.length);
  // супруги в одной строке Y и на соседних слотах
  const pos = new Map(persons.map(n => [n.id, n.position]));
